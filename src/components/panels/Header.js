...
import DarkLogo from "../../assets/images/vectors/logo.svg";
...
class Header extends Component {
  ...
  menuClicked(e) {
    let path1 = document.querySelector("#Path_1");
    let path2 = document.querySelector("#Path_2");
    let path3 = document.querySelector("#Path_3");
    let mobileMenu = document.querySelector("#mobile-menu");

    if (getComputedStyle(path2).getPropertyValue("opacity") !== "0") {
      path1.classList.add("menu-path-1");
      path2.classList.add("menu-path-2");
      path3.classList.add("menu-path-3");
      mobileMenu.setAttribute("class", "mobile-menu-open");
    } else {
      path1.classList.remove("menu-path-1");
      path2.classList.remove("menu-path-2");
      path3.classList.remove("menu-path-3");
      ...
      >
        <div id="header-left">
          ...
          <ul>
            {this.props.links.map((element, index) => {
              return (
                <PanelLink key={index} to={element.to} text={element.text} />
              );
            })}
          </ul>
        </div>
        <div id="header-right">
          <PanelLink to="#" text="Login" hoverColor="none" />
          <Button text="Sign Up" type="rounded" size="small" />
          <svg
            id="menu-btn"
            width="50"
            height="50"
          ...
          >
            ...
            <g id="burger-menu" clipPath="url(#clip-burger-menu)">
              <path
                id="Path_1"
                dataName="Path 1"
                d="M1608.327,151.719h30.566"
                transform="translate(-1598.61 -139.08)"
                fill="none"
                stroke="#232127"
                strokeWidth="2"
              />
              <path
                ...
                dataName="Path 2"
                d="M1608.327,151.719h30.566"
                transform="translate(-1598.61 -114.357)"
                fill="none"
                stroke="#232127"
                strokeWidth="2"
              />
              <path
                ...
                dataName="Path 3"
                d="M1608.327,151.719h30.566"
                transform="translate(-1598.61 -126.719)"
                fill="none"
                stroke="#232127"
                strokeWidth="2"
              />
        ...
        <div id="mobile-menu">
          <ul>
            {this.props.links.map((element, index) => {
              return (
                <PanelLink hoverColor="primary" key={index} to={element.to} text={element.text} />
              );
            })}
          </ul>
